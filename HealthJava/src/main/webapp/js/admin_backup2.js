  ...
  $.ajax({//$는 jQuery란 뜻. $.ajax 뜻은 jQuery 내의 아작스 실행
      ...
      success: function (res) {	
		 ...
		 for(let i = 0; i < res.length; i++){
			 ...
			 <td>${numberWithCommas(res[i].product_price)} 원</td>
...
function GsQNA_search(){
  ...
  $.ajax({//$는 jQuery란 뜻. $.ajax 뜻은 jQuery 내의 아작스 실행
      url:"/GsQNA_search", //url 패턴 매핑주소 경로
      type:"POST",//데이터를 서버로 보내는 방법
      data: {
      	"search_field" : searchField,
      	"search_type" : searchType
      ...
      success: function (res) {	
			 document.getElementById('ajaxTable').innerHTML = "";	
		 for(let i = 0; i < res.length; i++){
		 var QNAReply = res[i].qna_reply;
			 const html = `
				<tr>
  			 <td>${res[i].qna_no}</td>
             ${QNAReply !== ' ' ? '<td>답변완료</td>' : '<td>대기</td>'}
			 <td>${res[i].qna_product_no}</td>
			 <td onclick= "location.href ='admin_GsQNA_cont?qna_no=${res[i].qna_no}&qna_mem_id=${res[i].qna_mem_id}';">${res[i].qna_title} </td>
			 <td>${res[i].qna_mem_id}</td>
			 <td>${res[i].qna_date.substring(0,10)}</td>
			 </tr>
			 `
             //통신 성공시 탭 내용담는 div를 읽어들인 값으로 채운다.
             $('#ajaxTable').append(html); 
