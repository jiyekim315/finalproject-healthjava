...
<title>HealthJava 관리자 상품 문의 게시판</title>
				...
				<table align="center">
					...
					<td align="center">
							<select name="search_type" id="search_type">
								...
								<c:if test="${search_type == 'qna_title' }">${'selected' }</c:if>>제목</option>
								<option value="user_id" 
								<c:if test="${search_type == 'qna_mem_id' }">${'selected' }</c:if>>작성자</option>
							</select>
								<input id="search_field" name="search_field" onkeyup="GsQNA_search();" size="20"> &nbsp;&nbsp;&nbsp;
								<input type="button" id="btn" name="search" onclick="GsQNA_search();" value="검색"  />
						</td>
					</tr>
					</table><p> <%-- end 검색 테이블 --%>
					
					<div align="right">총 게시물 : ${GsQNA_count} 개</div><p>
